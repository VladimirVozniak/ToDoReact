...
const ToDoList = () => {
    const {todo, removeTodoElem, editTodoElem, checkedTodoElem} = useContext(Context)
    return (
        <>
            {todo.map((elem, index) =>
                <div key={index} className="todo-element">
                    <input type="checkbox"
                           ...
                           onChange={() => checkedTodoElem(index)}
                           className="checkbox"/>
                    <input className={elem.isChecked ? 'todo-text todo-text_disable' : 'todo-text'}
                           value={elem.value}
                           onChange={(e) => editTodoElem(e.target.value, index)}
                           disabled={elem.isChecked}/>
                    <button className="delete-icon"
                            onClick={() => removeTodoElem(index)}>
