...
import {checked, edit, reloadData, remove} from "../../Redux/todoSlice";
                ...
                <Spin key={index} spinning={elem.load}>
                    <div className="todo-element">
                        <input type="checkbox"
                               ...
                               onChange={async (e) => {
                                   dispatch(reloadData(elem._id))
                                   await updateTodo(elem._id, {isChecked: e.target.checked})
                        ...
                        <input className={elem.isChecked ? 'todo-text todo-text_disable' : 'todo-text'}
                               ...
                               onBlur={async () => {
                                   dispatch(reloadData(elem._id))
                                   await updateTodo(elem._id, {value: elem.value})
                                   dispatch(reloadData(elem._id))
                               }}
